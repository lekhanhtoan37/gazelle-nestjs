...
func (lang *NestJS) Imports(c *config.Config, r *rule.Rule, f *rule.File) []resolve.ImportSpec {
	...
	folderImports := nestjsConfig.CollectAll && (r.Kind() == getKind(c, "ts_project") || r.Kind() == getKind(c, "js_library"))
	if folderImports {
...
func (lang *NestJS) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, _imports interface{}, from label.Label) {
	...
	for name := range imports.set {
		...
		// fix aliases
		match := nestjsConfig.ImportAliasPattern.FindStringSubmatch(name)
		if len(match) > 0 {
			prefix := match[0]
			alias := ""
			for _, impAlias := range nestjsConfig.ImportAliases {
		...
		isNpm, npmLabel, devDep := lang.isNpmDependency(name, nestjsConfig)
		if isNpm {
			...
			if nestjsConfig.LookupTypes && r.Kind() == "ts_project" {
				...
				typesFound, npmLabel, _ := lang.isNpmDependency("@types/"+name, nestjsConfig)
				if typesFound {
		...
		if _, ok := BUILTINS[name]; ok || strings.HasPrefix(name, "node:") {
			...
			if nestjsConfig.LookupTypes && r.Kind() == "ts_project" {
				typesFound, npmLabel, _ := lang.isNpmDependency("@types/node", nestjsConfig)
				if typesFound {
...
func (lang *NestJS) resolveWalkParents(name string, depSet map[string]bool, dataSet map[string]bool, c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, from label.Label) {
	...
	for {
		...
		if !lang.isWebAsset(nestjsConfig, target) {
		...
		for _, ext := range extraExtensionsToTry {
			...
			if resolveResult.label != label.NoLabel {
				...
				lbl := resolveResult.label
				dep := lbl.Rel(from.Repo, from.Pkg).String()
				if !lang.isWebAsset(nestjsConfig, filePath) {
					depSet[dep] = true
				} else {
					dataSet[dep] = true
				}
...
func (lang *NestJS) isNpmDependency(imp string, nestjsConfig *NestjsConfig) (bool, string, bool) {
	...
	if npmLabel, ok := nestjsConfig.NpmDependencies.Dependencies[packageRoot]; ok {
		return true, npmLabel, false
	}

	if npmLabel, ok := nestjsConfig.NpmDependencies.DevDependencies[packageRoot]; ok {
	...
	if strings.HasPrefix(imp, "@") {
		return true, nestjsConfig.DefaultNpmLabel, false
	}
...
func (lang *NestJS) tryResolve(target string, c *config.Config, ix *resolve.RuleIndex, from label.Label) resolveResult {
