...
func (ucr *updateConfigurer) RegisterFlags(fs *flag.FlagSet, cmd string, c *config.Config) {
	...
	fs.BoolVar(
		...
		"r",
		true,
		"when true, gazelle will update subdirectories recursively",
	)
	fs.StringVar(
		&uc.patchPath,
		"patch",
		"",
		"when set with -mode=diff, gazelle will write to a file instead of stdout",
	)
	fs.BoolVar(
		&uc.print0,
		"print0",
		false,
		"when set with -mode=fix, gazelle will print the names of rewritten files separated with \\0 (NULL)",
	)
	...
	fs.Var(
		&gzflag.MultiFlag{Values: &ucr.knownImports},
		"known_import",
		"import path for which external resolution is skipped (can specify multiple times)",
	)
	fs.StringVar(
		&ucr.repoConfigPath,
		"repo_config",
		"",
		"file where Gazelle should load repository configuration. Defaults to WORKSPACE.",
	)
}
...
func runFixUpdate(wd string, cmd command, args []string) (err error) {
		...
		func(dir, rel string, cfg *config.Config, update bool, ruleFile *rule.File, subdirs, regularFiles, genFiles []string) {
			...
			for _, l := range filterLanguages(cfg, languages) {
				...
				if len(res.Gen) != len(res.Imports) {
					log.Panicf(
						"%s: language %s generated %d rules but returned %d imports",
						rel,
						l.Name(),
						len(res.Gen),
						len(res.Imports),
			...
			for _, r := range allRules {
				...
				if err != nil {
					errorsFromWalk = append(
						errorsFromWalk,
						fmt.Errorf("looking up mapped kind: %w", err),
					)
					continue
				}
				if repl != nil {
	...
	if len(errorsFromWalk) > 1 {
		...
		return fmt.Errorf(
			"encountered multiple errors: %w, %v",
			errorsFromWalk[0],
			strings.Join(additionalErrors, ", "),
		)
	}
...
func lookupMapKindReplacement(
	kindMap map[string]config.MappedKind,
	kind string,
) (*config.MappedKind, error) {
	...
	for {
		...
		if _, alreadySeen := seenKinds[replacement.KindName]; alreadySeen {
			return nil, fmt.Errorf(
				"found loop of map_kind replacements: %s",
				strings.Join(seenKindPath, " -> "),
			)
		}
...
func findOutputPath(c *config.Config, f *rule.File) string {
	...
	outputPath := rule.MatchBuildFileName(outputDir, c.ValidBuildFileNames, fileInfos)
	if outputPath == "" {
		return defaultOutputPath
	}
	return outputPath
...
func appendOrMergeKindMapping(
	mappedLoads []rule.LoadInfo,
