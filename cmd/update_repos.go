...
import (
	...
	"github.com/lekhanhtoan37/gazelle-nestjs/internal/module"
	"github.com/lekhanhtoan37/gazelle-nestjs/internal/wspace"
)
...
func (f macroFlag) Set(value string) error {
	...
	if len(args) != 2 {
		return fmt.Errorf(
			"Failure parsing to_macro: %s, expected format is macroFile%%defName",
			value,
		)
	}
	if strings.HasPrefix(args[0], "..") {
		return fmt.Errorf(
			"Failure parsing to_macro: %s, macro file path %s should not start with \"..\"",
			value,
			args[0],
		)
	}
...
func (*updateReposConfigurer) RegisterFlags(fs *flag.FlagSet, cmd string, c *config.Config) {
	...
	fs.StringVar(
		...
		"from_file",
		"",
		"Gazelle will translate repositories listed in this file into repository rules in WORKSPACE or a .bzl macro function. Gopkg.lock and go.mod files are supported",
	)
	fs.Var(
		macroFlag{macroFileName: &uc.macroFileName, macroDefName: &uc.macroDefName},
		"to_macro",
		"Tells Gazelle to write repository rules into a .bzl macro function rather than the WORKSPACE file. . The expected format is: macroFile%defName",
	)
	fs.BoolVar(
		&uc.pruneRules,
		"prune",
		false,
		"When enabled, Gazelle will remove rules that no longer have equivalent repos in the go.mod file. Can only used with -from_file.",
	)
}
...
func (*updateReposConfigurer) CheckFlags(fs *flag.FlagSet, c *config.Config) error {
	...
	case uc.repoFilePath != "":
		if len(fs.Args()) != 0 {
			return fmt.Errorf(
				"got %d positional arguments with -from_file; wanted 0.\nTry -help for more information.",
				len(fs.Args()),
			)
		}
...
func newUpdateReposConfiguration(
	wd string,
	args []string,
...
func ensureMacroInWorkspace(uc *updateReposConfig, insertIndex int) (updated bool) {
	...
	for _, d := range uc.workspace.Directives {
		if d.Key == "repository_macro" {
			if parsed, _ := repo.ParseRepositoryMacroDirective(d.Value); parsed != nil &&
				parsed.Path == uc.macroFileName &&
				parsed.DefName == uc.macroDefName {
				return false
			}
		}
	}
